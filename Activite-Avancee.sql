...
-- 1 - nombre total de séances créées
...
-- 2 - nombre de séances réservées
SELECT COUNT(*) AS seances_reservees FROM reservations;
+-------------------+
| seances_reservees |
+-------------------+
|                 8 |
...
-- 3 - seulement les coachs ayant ≥3 séances
SELECT coach_id, COUNT(coach_id) AS coach_seances FROM seances GROUP BY coach_id HAVING COUNT
(coach_id) >= 3;
+----------+---------------+
| coach_id | coach_seances |
+----------+---------------+
|        1 |             3 |
|        2 |             3 |
+----------+---------------+
